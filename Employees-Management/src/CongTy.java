...
public class CongTy {
	...
	public void nhap(Scanner scan) {
		System.out.println("******* Insert Information of Company *********");
	...
	public void xuat() {
		...
		System.out.println("Tax number: " + this.maSoThue);
		System.out.println("Monthly Revenue: " + this.doanhThuThang);
		System.out.println("**************************************");
	...
	public void dummyDataNhanSu() {
		...
		objListNhanSu.them(ns);
		ns = new NhanVienThuong("1", "Laury", "09832", 20);
		objListNhanSu.them(ns);
		ns = new NhanVienThuong("2", "Henry", "098232", 23);
		objListNhanSu.them(ns);
		ns = new NhanVienThuong("3", "Vivivan", "098132", 31);
		objListNhanSu.them(ns);
		ns = new NhanVienThuong("4", "Anne", "091832", 25);
		objListNhanSu.them(ns);
		ns = new NhanVienThuong("5", "Thod", "098432", 23);
		objListNhanSu.them(ns);
		ns = new NhanVienThuong("6", "Mina", "092832", 31);
		objListNhanSu.them(ns);

		ns = new TruongPhong("8", "Mary", "098233", 24);
		objListNhanSu.them(ns);
		ns = new TruongPhong("9", "Luke", "091833", 21);
		objListNhanSu.them(ns);
		ns = new TruongPhong("10", "Kate", "091833", 23);
		objListNhanSu.them(ns);

		ns = new GiamDoc("11", "Taylor", "0981", 19, 80);
		objListNhanSu.them(ns);
		ns = new GiamDoc("12", "Hush", "09181", 21, 20);
		objListNhanSu.them(ns);
	}
	...
	public void phanBoNhanVienThuong() {
		...
		for (NhanSu ns : objListNhanSu.getListNhanSu()) {
			if (ns instanceof NhanVienThuong) {
				...
				System.out.println("Allocating staffs:");
				ns.xuatMaVaTen();
	...
	private void phanBoChoMotNhanSu(Scanner scan, NhanSu ns) {
		...
		do {
			// Bước 3
			System.out.println("Choose 1 for allocate, 2 for continue");
	...
	private TruongPhong chonTruongPhong(Scanner scan) {
		TruongPhong tp = null;
		do {
			System.out.println("Please choose Manager's code above");
	...
	public void themNhanSu(Scanner scan) {
		System.out.println("****** Adding staff funtion ******");
	...
	public boolean xoaNhanSu(Scanner scan) {
		...
		System.out.println("********** Deleting staff function ***********");
		System.out.println("Give the code of staff need to be deleted");
		...
		if (nhanSu != null) { // Tìm thấy và xử lý
			...
			System.out.println("Staff's code is not available");
		}
